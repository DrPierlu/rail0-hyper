...
RAIL0.PaymentAuthorized.handler(async ({ event, context }) => {
  ...
  await notifyAndRecord(
    ...
    evId,
    id,
    { eventType: "authorized", chainId, paymentId: paymentId as `0x${string}`, blockNumber: event.block.number },
...
RAIL0.PaymentCharged.handler(async ({ event, context }) => {
  ...
  await notifyAndRecord(
    ...
    evId,
    id,
    { eventType: "charged", chainId, paymentId: paymentId as `0x${string}`, blockNumber: event.block.number },
    event.transaction.hash,
    event.block.timestamp,
  );
});
...
RAIL0.PaymentCaptured.handler(async ({ event, context }) => {
  ...
  await notifyAndRecord(
    ...
    evId,
    id,
    { eventType: "captured", chainId, paymentId: paymentId as `0x${string}`, blockNumber: event.block.number, amount: amount.toString() },
    event.transaction.hash,
    event.block.timestamp,
  );
});
...
RAIL0.PaymentVoided.handler(async ({ event, context }) => {
  ...
  await notifyAndRecord(
    ...
    evId,
    id,
    { eventType: "voided", chainId, paymentId: paymentId as `0x${string}`, blockNumber: event.block.number, amount: amount.toString() },
    event.transaction.hash,
    event.block.timestamp,
  );
});
...
RAIL0.PaymentReleased.handler(async ({ event, context }) => {
  ...
  await notifyAndRecord(
    ...
    evId,
    id,
    { eventType: "released", chainId, paymentId: paymentId as `0x${string}`, blockNumber: event.block.number, amount: amount.toString() },
    event.transaction.hash,
    event.block.timestamp,
  );
});
...
RAIL0.PaymentRefunded.handler(async ({ event, context }) => {
  ...
  await notifyAndRecord(
    ...
    evId,
    id,
    { eventType: "refunded", chainId, paymentId: paymentId as `0x${string}`, blockNumber: event.block.number, amount: amount.toString() },
    event.transaction.hash,
    event.block.timestamp,
  );
});
